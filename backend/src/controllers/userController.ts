...
export const signup = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      res.status(400).json({ success: false, error: 'Format d\'email invalide' });
      return;
    }
    ...
    const [existingEmail] = await pool.query<User[]>('SELECT id_user FROM USERS WHERE email = ?', [email]);
    if (existingEmail.length > 0) {
    ...
    const [existingUsername] = await pool.query<User[]>('SELECT id_user FROM USERS WHERE username = ?', [username]);
    if (existingUsername.length > 0) {
...
export const deleteUser = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const [users] = await pool.query<User[]>('SELECT id_user FROM USERS WHERE id_user = ?', [id]);
    if (users.length === 0) {
...
export const getAllUsers = async (req: Request, res: Response): Promise<void> => {
  try {
    const [users] = await pool.query<User[]>(
      'SELECT id_user, username, email FROM USERS ORDER BY id_user'
    );
